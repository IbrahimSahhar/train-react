...
import { POSTS_ACTIONS } from "../Constants/postsAction";
import { posts } from "../mock/postsData";
...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case POSTS_ACTIONS.SELECT: // Get Single Post
      return {
        ...state,
        post: state.posts.find((item) => item.id === action.payload),
      };
...
const usePosts = () => {
  ...
  const createPost = (body) =>
    dispatch({ type: POSTS_ACTIONS.CREATE, payload: body });

  const updatePost = (body) =>
    dispatch({ type: POSTS_ACTIONS.UPDATE, payload: body });
